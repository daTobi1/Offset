    ...
    <div class="row">
      ...
      <div class="col-6">
        <div class="border border-secondary-subtle rounded p-2 bg-dark h-100 d-flex flex-column justify-content-center">
          ...
          <button type="button" class="btn btn-sm btn-secondary fs-6 border text-center w-100 ${disabled}"
                  style="padding-bottom:10px; padding-top:10px;" id="capture-pos">
            CAPTURE <br/> CURRENT <br/> POSITION
          </button>
          <small class="text-secondary mt-2">Tip: switch to Master tool first (tool must be active).</small>
        </div>
      </div>
      <div class="col-4">
        <div class="border border-secondary-subtle rounded p-2 bg-dark h-100">
          ...
          <div class="row"><div class="col-4"><small>X:</small></div><div class="col-8 text-end"><span id="captured-x"><small></small></span></div></div>
          <div class="row"><div class="col-4"><small>Y:</small></div><div class="col-8 text-end"><span id="captured-y"><small></small></span></div></div>
          <div class="row"><div class="col-4"><small>Z:</small></div><div class="col-8 text-end"><span id="captured-z"><small></small></span></div></div>
          <hr class="my-2"/>
          <div class="row">
          ...
          <hr class="my-2"/>
          <button type="button" class="btn btn-outline-light btn-sm w-100 copy-offset-btn"
                  data-tool="${tool_number}" data-gcode-x-offset="${cx_offset}"
                  data-gcode-y-offset="${cy_offset}" data-gcode-z-offset="${cz_offset}">
            Copy Offsets
          </button>
        </div>
      </div>
      ...
      <div class="col-6">
        <div class="row pb-3">
          <div class="input-group ps-1 pe-1">
            ...
            <input type="number" name="T${tool_number}-x-pos" class="form-control" placeholder="0.0"
                   data-axis="x" data-tool="${tool_number}" ${disabled}>
          </div>
        </div>
        <div class="row">
          <div class="input-group ps-1 pe-1">
            ...
            <input type="number" name="T${tool_number}-y-pos" class="form-control" placeholder="0.0"
                   data-axis="y" data-tool="${tool_number}" ${disabled}>
          </div>
        </div>
      </div>
...
// FIX: expose stop so disconnect in index.js can clean up stale interval
function stopProbeResultsUpdates() {
  if (_probeInterval) { clearInterval(_probeInterval); _probeInterval = null; }
}

function normalizeOffsetValue(value) {
  const parsed = parseFloat(value);
  return Number.isNaN(parsed) ? "0.000" : parsed.toFixed(3);
}
...
<li class="list-group-item bg-body-tertiary p-2">
  <div class="container">
      ...
      <div class="col-12">
        <div class="border border-secondary-subtle rounded p-2 bg-dark">
          ...
          <select id="z-calc-method" class="form-select form-select-sm w-auto d-inline-block">
            <option value="config"  ${sel === 'config'   ? 'selected' : ''}>${cfgLabel}</option>
            <option value="median"  ${sel === 'median'   ? 'selected' : ''}>Median</option>
            <option value="average" ${sel === 'average'  ? 'selected' : ''}>Average</option>
            <option value="trimmed" ${sel === 'trimmed'  ? 'selected' : ''}>Trimmed mean</option>
          </select>
        </div>
      </div>
    </div>
    <div class="row">
      <div class="col-12">
        <button class="btn ${btnClass} w-100" id="calibrate-all-btn" ${disabledAttr}>CALIBRATE Z-OFFSETS</button>
      </div>
    </div>
  </div>
</li>`;
...
$(document).on("click", "#calibrate-all-btn", function() {
  const selectedTools = $(".calibrate-tool-checkbox:checked")
    .map(function(){ return parseInt(this.value, 10); }).get().filter(v => !Number.isNaN(v));
...
$(document).on("change", "#calibrate-select-all", function () {
  ...
  $(`#calibrate-tool-${getSelectedReferenceTool(0)}`).prop("checked", true);
  syncSelectAllState();
});
...
function toolChangeURL(tool) {
  ...
  if (Number.isNaN(x_pos) || Number.isNaN(y_pos) || Number.isNaN(z_pos)) {
    return printerUrl(printerIp, "/printer/gcode/script?script=OFFSET_BEFORE_PICKUP_GCODE%0AT" + tool + "%0AOFFSET_AFTER_PICKUP_GCODE");
  }
  ...
  if (String(tool) !== String(master)) {
    ...
    if (tool_x !== 0.0 && tool_y !== 0.0) { x_pos = tool_x; y_pos = tool_y; }
  }

  let url = printerUrl(printerIp, "/printer/gcode/script?script=OFFSET_BEFORE_PICKUP_GCODE");
  url += "%0AT" + tool + "%0AOFFSET_AFTER_PICKUP_GCODE";
  url += "%0ASAVE_GCODE_STATE NAME=RESTORE_POS%0AG90";
  url += "%0AG0 Z" + z_pos.toFixed(3) + " F3000";
  url += "%0AG0 X" + x_pos.toFixed(3) + " Y" + y_pos.toFixed(3) + " F12000";
...
function getTools() {
  $.get(printerUrl(printerIp, "/printer/objects/query?toolchanger"))
    .done(function(data){
      ...
      $.get(printerUrl(printerIp, queryUrl)).done(function(toolData){
        $("#tool-list").html("");
...
function updateOffset(tool, axis) {
  ...
  if (position !== 0.0 && capturedText !== "") {
    ...
    $newEl.attr("data-raw", new_offset.toFixed(3)).find(">:first-child").text(new_offset.toFixed(3));
  } else {
    $newEl.attr("data-raw", "0.000").find(">:first-child").text("0.0");
  }
...
function updateTools(tool_numbers, tool_number_active) {
  ...
  if ($captureBtn.length) {
    if (activeTool !== parseInt(master, 10)) $captureBtn.addClass("disabled").prop("disabled", true);
    else $captureBtn.removeClass("disabled").prop("disabled", false);
  }

  (tool_numbers || []).forEach((tool_no) => {
    const isActive = parseInt(tool_no, 10) === activeTool;
    $(`#T${tool_no}-fetch-x, #T${tool_no}-fetch-y`).toggleClass("disabled", !isActive).prop("disabled", !isActive);
    $(`input[name=T${tool_no}-x-pos], input[name=T${tool_no}-y-pos]`).prop("disabled", !isActive);
    $(`button#toolchange[data-tool='${tool_no}']`).toggleClass("disabled", isActive).prop("disabled", isActive);
  });
